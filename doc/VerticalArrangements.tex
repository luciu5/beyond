...
The paper proceeds as follows.  In Section \ref{sec:theory}, we introduce the theoretical framework and explain how we model mergers.  Section \ref{sec:sims} explains how we construct our numerical simulations and provides our results, and our application to Republic/Santek appears in Section \ref{sec:application}.  Section \ref{sec:concl} concludes.  Additional simulation results appear in the Appendix.
...
The vertical supply chain in the solid waste industry is primarily comprised of waste collection operations or ``haulers'' and waste disposal facilities. Haulers collect MSW from businesses and residences and must dispose of it at a lawful disposal site, usually a landfill. Waste disposal (upstream) is a required input into waste collection services (downstream). Some haulers are vertically integrated and operate their own disposal facilities. Vertically-integrated haulers typically prefer to dispose of waste at their own disposal facilities and may also sell a portion of their disposal capacity. Disposal customers include private waste haulers without their own disposal assets and as well as local governments that collect their citizens' waste themselves. Due to strict laws and regulations that govern the disposal of MSW, there are no reasonable substitutes for MSW disposal.
...
Turning to the downstream market, all collection prices increase post-merger. The merging parties' post-merger collection prices increase by approximately 20\% and share decreases by about 8 percentage points overall. The Republic previously integrated product's price increases 21\% and the Santek previously integrated product's price increases by 23\%. The newly integrated Santek-Republic product's price is estimated to increase by 9\%, demonstrating that EDM is not fully passed-through to consumers. The integrated rival, Waste Connections, increases its collection price by approximately 5\% and its share increases by 6 percentage points. The unintegrated rivals' collection prices each increase by about 9\%, which again demonstrates incomplete pass-through of the change in upstream costs, and their shares increase slightly.

In Figure \ref{fig:TrashSimsCompareAll} we compare the full vertical model results, which computes both post-merger downstream and upstream price effects, to two other vertical supply chain models that hold the prices of one level fixed, treating each upstream-downstream pair as if it were an independent firm.\footnote{This approach uses the same calibrated demand and costs parameters across all three models.} The ``Downstream Only'' model computes collection price effects holding disposal prices fixed at pre-merger levels, and the ``Upstream Only'' model computes dosposal price effects holding collection prices fixed at pre-merger levels.

Ignoring these vertical relationships in the downstream-only model results in an estimated 11\% increase in collection prices with a greater than 16\% increase in the merging parties' price, consumer harm of \$10 million annually and net harm of approximately \$2 million. Estimated consumer harm is 60\% higher in the model including vertical relationships and is offset by markedly larger producer benefits. The collection-only model misses the substantial RRC effect experienced by the Santek-WM-ADS and Santek-Regional products.  In the vertical model, the downstream prices for these products rise more dramatically due to this RRC, and other prices increase alongside them.  This causes more harm to consumers.

In the upstream-only model, however, downstream prices and therefore shares are fixed at their pre-merger levels. While disposal prices increase by 8.2\% post-merger the merger effect is a pure transfer between disposal and collection firms. Thus, both consumer and net harm are estimated to be zero.
...
\centering
\includegraphics[scale=0.9]{surplussum.png}
...
\begin{sidewaysfigure}
\centering
\includegraphics[scale=0.9]{CVvertincumbBW_consumer.png}
...
\begin{sidewaysfigure}
\centering
\includegraphics[scale=0.9]{CVbargincumbent_consumer.png}
...
\begin{figure}
\centering
\includegraphics[scale=1.0]{chattanooga_tree.png}
...
\begin{figure}
\centering
\includegraphics[scale=1.0]{TrashSimsFirm.png}
...
\begin{figure}
\centering
\includegraphics{TrashSimsCompare.png}
...
\begin{figure}
\centering
\includegraphics{TrashSimsCompareAll.png}
\caption{Comparison between Collection-Only, Disposal-only and Full Vertical Simulations for Republic/Santek}
...
\begin{sidewaysfigure}
\centering
\includegraphics[scale=0.9]{surplussum_cost.png}
...
\begin{sidewaysfigure}
\centering
\includegraphics[scale=0.9]{CVvertincumbBW_total.png}
...
\begin{sidewaysfigure}
\centering
\includegraphics[scale=0.9]{CVbargincumbent_total.png}
