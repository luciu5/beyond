---
title: "Landfill Geographic Analysis: Operator Level"
...
disposal_df <- raw %>%
  select(Code,Location,City,County,State,Zip,Longitude,Latitude,Code,Type,Family,`Waste Shed`,`Operator Code`,`Total of All Wastes (tons/day)`,Operation) %>% # Make changes to entity type here
...
landfill_csa_df <- intersect_df %>%
  ...
  mutate(firm_name="") %>%
  select(Operator_Code,market,st_cnty,County,State,`Total of All Wastes (tons/day)`,iscsa) %>% # Change this line
  group_by(Operator_Code,market,st_cnty,County,State,`Total of All Wastes (tons/day)`,iscsa) %>% # Change this line
...
landfill_csa_df <- intersect_df %>%
  ...
  filter(Waste_Shed!="Exclusive or Captive"|is.na(Waste_Shed)) %>% # Step 2: Drop all observations labelled "Exclusive or Captive" (1619 Observations)
  mutate(firm_name="") %>%
  select(Operator_Code,market,st_cnty,County,State,`Total of All Wastes (tons/day)`,iscsa,Operation) %>% # Change this line
  ungroup()
...
landfill_cbsa_df <- cbsa_intersect_df %>%
  ...
  filter(Waste_Shed!="Exclusive or Captive"|is.na(Waste_Shed)) %>% # Step 2: Drop all observations labelled "Exclusive or Captive" (1619 Observations)
  select(Operator_Code,market,st_cnty,County,State,`Total of All Wastes (tons/day)`,iscbsa) %>% # Change this line
  ungroup()
...
This analysis is done on the Operator Code level or Operator Entity name if Operator Code does not exist.  Let's first look at number of markets that are in a CSA and number of markets that are county based as they do not fall in a CSA.  We see that only `r paste0(round(sum_landfill$per_count[1],0),"%")` of landfills are CSA market based
```{r}
...
landfill_shrs_csa <- landfill_csa_df %>%
  ungroup() %>%
  group_by(Operator_Code,market,iscsa) %>% # Aggregate on Market level, Owner Entity, Operator level
...
landfill_shrs_cbsa <- landfill_cbsa_df %>%
  ungroup() %>%
  group_by(Operator_Code,market,iscbsa) %>% # Aggregate on Market level, Owner Entity, Operator level
  summarise(waste_own = sum(`Total of All Wastes (tons/day)`), .groups="keep") %>%
  ungroup() %>%
  group_by(market) %>% # Aggregate on Market
  mutate(total_waste = sum(waste_own),
...
Now let's look at the tabulation of number of firms per market (assuming equally sized firms) from the HHIs we calculated before.  We will use the ceiling function to round to the nearest integer.

```{r}
# summary(non_mono_and_mono_landfill_csa_df$waste_hhi)
non_mono_and_mono_landfill_csa_df <- landfill_csa %>% 
  mutate(eff_firms = 1e4/waste_hhi,
         old_eff_firms=eff_firms,
         old_eff_firms=ceiling(eff_firms),
         eff_firms = ifelse((ceiling(eff_firms)-eff_firms)>0.5,floor(eff_firms),ceiling(eff_firms)))
#1.8 median means there are 1.8 equally sized firms in the market (equivalent)
```


```{r}
data.frame(table(non_mono_and_mono_landfill_csa_df$old_eff_firms)) %>%
  pivot_wider(names_from=Var1, values_from=Freq) %>% 
  kbl(full_wdith=F,
      digits=0,
      caption = "Tabulations if Firms were equally sized (Based on HHIs)") %>%
  kable_classic_2(full_width = F)
```

Now let's do a similar exericse but correcting for rounding issues.

```{r}
data.frame(table(non_mono_and_mono_landfill_csa_df$eff_firms)) %>%
  pivot_wider(names_from=Var1, values_from=Freq) %>% 
  kbl(full_wdith=F,
      digits=0,
      caption = "Tabulations if Firms were equally sized (Based on HHIs. Corrected for rounding)") %>%
  kable_classic_2(full_width = F)
```
...
Now let's look at the tabulation of number of firms per market (assuming equally sized firms) from the HHIs we calculated before like we did in the previous section.  Notice that the tabulations are quite similar when we looked at all markets.  The first table calculates this based on a ceiling function while the second one calculates this with some correction for rounding (using either a floor or ceiling function dependent on the number)

```{r}
# summary(non_mono_landfill_csa_df$waste_hhi)
non_mono_landfill_csa_df <- non_mono_landfill_csa_df %>% 
  mutate(eff_firms = 1e4/waste_hhi,
         old_eff_firms=eff_firms,
         old_eff_firms=ceiling(eff_firms),
         eff_firms = ifelse((ceiling(eff_firms)-eff_firms)>0.5,floor(eff_firms),ceiling(eff_firms)))
...
```{r}
data.frame(table(non_mono_landfill_csa_df$old_eff_firms)) %>%
  pivot_wider(names_from=Var1, values_from=Freq) %>% 
  kbl(full_wdith=F,
      digits=0,
      caption = "Tabulations if Firms were equally sized (Based on HHIs)") %>%
...
```{r}
data.frame(table(non_mono_landfill_csa_df$eff_firms)) %>%
  pivot_wider(names_from=Var1, values_from=Freq) %>% 
  kbl(full_wdith=F,
      digits=0,
      caption = "Tabulations if Firms were equally sized (Based on HHIs. Corrected for rounding)") %>%
...
chat_sum <- chat_shrs %>%
  ungroup() %>%
  select(Operator_Code,waste_own,waste_shr,waste_hhi)
...
```{r}
chat_sum %>%
  select(Operator_Code,waste_own,waste_shr) %>%
  arrange(desc(waste_shr)) %>%
  kbl(col.names=c("Operator Code","Total Waste","Waste Share"),
      digits=0,
      caption = "# Landfills by Markets") %>%
  kable_classic_2(full_width = F)
```
...
chk_vertical <- disposal_df %>%
  filter(Type=="LF"|Family=="LF"|Type=="TS"|Family=="TS") %>%
  mutate(is_municp=grepl("COUNTY|CITY|TOWN|VILLAGE",Operator_Code),
...
landfill_shrs_csa_operation <- landfill_csa_df %>%
  ungroup() %>%
  group_by(Operator_Code,market,iscsa,Operation) %>% # Aggregate on Market level, Owner Entity, Operator level
  summarise(waste_own = sum(`Total of All Wastes (tons/day)`), .groups="keep") %>%
  ungroup() %>%
  group_by(market) %>% # Aggregate on Market
  mutate(total_waste = sum(waste_own),
