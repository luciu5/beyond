...
basedir <- file.path(Sys.getenv("HOME"), "Projects", "beyond")
batchdir <- file.path(basedir, "data", "batch")
simpath <- file.path(basedir, "data")
file.remove(file.path(basedir, "code", "beyond_slurm.out"))
...
res.nests_plan <- expand_grid(
  Grp = multiplier, samples,
  up = ndfirms,
  down = nufirms,
  vert = nvfirms,
  hhidelta = hhidelta,
...
res.nests_plan <- mutate(res.nests_plan,
  ...
  filter(!(merger %in% c("up") & (up %in% 1:2))) %>%
  filter(!(merger %in% c("down") & (down %in% 1:2))) %>%
  filter(!(merger %in% c("both") & (down %in% 1))) %>%
...
write_csv(res.nests_plan, file = file.path(basedir, "data", "simjobs.csv"))
...
cat(
  file = file.path(basedir, "code", "runsims.sh"),
  sprintf(
    '#!/bin/bash
#SBATCH --job-name=beyond
#SBATCH --array=1-%s%s
#SBATCH --ntasks=1
#SBATCH --mem=1000m
...
#SBATCH -o beyond_slurm.out
#SBATCH --open-mode=append

rm ${HOME}/Projects/beyond/code/slurm/output_log.txt
cd ${HOME}/Projects/beyond
...
header=$(head -1  ${HOME}/Projects/beyond/data/simjobs.csv)
nlines=$(wc -l  < ${HOME}/Projects/beyond/data/simjobs.csv)
...
for (( run=$START_NUM; run<=$END_NUM; run++ )); do
  lineno=$(($run + 1))
  thisrun=$(head -n $lineno ${HOME}/Projects/beyond/data/simjobs.csv | tail -n 1)
  if  [[ -z "$thisrun" || $lineno -gt $nlines ]];
  then
    break;
  fi
  echo This is SLURM task $SLURM_ARRAY_TASK_ID, run number $run >> ${HOME}/Projects/beyond/code/slurm/output_log.txt

#echo "$lineno: $thisrun"
srun Rscript ${HOME}/Projects/beyond/code/slurm/BatchSims.R "$header" "$thisrun" "$lineno"
done
',
    ceiling(nrow(res.nests_plan) / chunk), paste0("%", cores), chunk
  )
)
