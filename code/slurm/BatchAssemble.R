...
profitableMarkets <- filter(res_mkt, isMarket) %>%
...
save(res_mkt, file = file.path(basedir, "data", "resultsMkt.RData"))
save(res_firms, file = file.path(basedir, "data", "resultsFirm.RData"))
...
res_mkt.long <- select(
  res_mkt, type, down, up, vert, merger, barg, nestParm, mc, cv, upPSDelta, downPSDelta, totalDelta, relleveragePre, # relmarginPreCut,
...
res_mkt_all.long <- select(
  res_mkt_all, type, down, up, vert, merger, barg, nestParm, mc, preset, cv, upPSDelta, downPSDelta, totalDelta, relleveragePre, # relmarginPreCut,
  avgpricedelta, mktrev.pre
) %>%
  rename(
    Consumer = cv, `Wholesaler` = upPSDelta, `Retailer` = downPSDelta, `Total` = totalDelta, Wholesalers = up,
...
sink(file.path(basedir, "doc", "sumtable.tex"))
