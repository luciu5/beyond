...
# setwd(basedir)
paired_data_4_22 <- read_csv(file.path(basedir, "trash", "paired_data_4_22.csv"))
...
simdata <- filter(paired_data_4_22, collection_paired_share > 0) %>%
  ...
  mutate(
    ...
    ownerPostDown = ifelse(ownerPreDown == "Santek", "Republic", ownerPreDown)
  ) %>%
  ungroup() %>%
  mutate(
...
for (v in vertFirms) {
  ...
  ownerPreUpMat[
    simdata$ownerPreUp == v & simdata$ownerPreDown != v,
    simdata$ownerPreUp == v & simdata$ownerPreDown == v
  ] <- 0
...
minD <- function(theta, is2nd = FALSE, margins = FALSE, nests, nestParm = 0, relevant) {
  ...
  for (v in vertFirms) {
    ...
    if (!is2nd) {
      ownerDownMatVertical[simdata$ownerPreDown == v, vertrows] <- 1
    }
  ...
  if (is2nd) {
    marginsCandDown <- simdata$marginDown_2nd * simdata$alpha_2nd[1] / alpha
  } else {
    marginsCandDown <- simdata$marginDown_bert * simdata$alpha_bert[1] / alpha
  }
  ...
  if (!missing(nests)) {
    ...
    if (any(class(elast.inv) == "try-error")) {
      elast.inv <- MASS::ginv(ownerPreDownMat * elast)
    }
  }
  ...
  if (!margins) {
    ...
    return(list(up = marginsCandUp, down = as.vector(marginsCandDown)))
  }
}
...
simdata <-
  mutate(simdata,
    ...
    disposal_cost_bert = disposal_price - marginUp_bert
  ) %>%
  group_by(disposal_firm_name) %>%
  mutate(
...
sink(file.path(basedir, "doc", "TrashData.tex"))
print(
  kable(
    select(
      simdata, disposal_firm_name,
      ...
      rename(
        "Disposal Firm" = disposal_firm_name,
...
vert_sum <- separate(vert_sum, name, sep = ":", into = c("Disposal", "Collector")) %>%
  ...
  mutate(
    Level = ifelse(grepl("Up", name), "Disposal", "Collection"),
  ...
  mutate(
    `Change (%)` = (`Post-merger` / `Pre-merger` - 1) * 100,
...
sink(file.path(basedir, "doc", "TrashSims.tex"))
kable(vert_sum,
  format = "latex",
  booktabs = TRUE,
...
vert_sum <- mutate(vert_sum,
  Name = interaction(Disposal, Collector, drop = TRUE, sep = "-"),
...
firmplot <- ggplot(
  data = vert_sum %>%
    ...
    mutate(
      Type = factor(Type, levels = c("Pre-merger", "Post-merger")),
      Name = factor(as.character(Name))
    ) %>%
    filter(!grepl("Change", Type)) %>% arrange(Name),
  aes(x = Name, y = value, fill = Type, label = value)
) +
  facet_grid(~ Level + Effect, scales = "free_x") +
  geom_bar(
    stat = "identity",
    position = position_dodge2(reverse = TRUE)
    # position="stack"
  ) +
  xlab("Disposal-Collector") +
  ylab("Equilibrium Levels") + # geom_hline(yintercept = 0,linetype="dashed") +
  ...
  geom_text( # color=ifelse(Type=="Post-merger","white","black"),
    hjust = 1.5, position = position_dodge2(width = .9, reverse = TRUE), size = 3
  ) +
  coord_flip() +
  theme_bw() +
...
compareplot <- ggplot(
  ...
  xlab("Disposal-Collector") +
  ylab("Equilibrium Prices") + # geom_hline(yintercept = 0,linetype="dashed") +
...
compareplot_noup <- ggplot(
  data = filter(compare, Type != "Pre-merger" & !(Level == "Disposal" & Effect == "Prices") &
    Model != "Upstream-only"),
  aes(x = Name, y = value, fill = Model, label = value)
) +
  facet_grid(~ Level + Effect, scales = "free_x") +
  ...
  xlab("Disposal-Collector") +
  ylab("Equilibrium Post-merger Levels") + # geom_hline(yintercept = 0,linetype="dashed") +
  scale_fill_brewer(type = "qual", palette = "Paired") + # scale_fill_grey(start = .9, end = .1) +
  geom_text( # color=ifelse(Type=="Post-merger","white","black"),
    hjust = 1.5, position = position_dodge(width = .9), size = 3
  ) +
  coord_flip() +
  theme_bw() +
  theme(legend.position = "bottom", axis.text.x = element_text(angle = 45, hjust = 1))
...
png(
  filename = file.path(basedir, "output", "TrashSimsFirm.png"), res = 250, units = "in",
  width = 6, height = 6
)
...
png(
  filename = file.path(basedir, "output", "TrashSimsCompare.png"), res = 250, units = "in",
  width = 6, height = 6
)
...
png(
  filename = file.path(basedir, "output", "TrashSimsCompareAll.png"), res = 250, units = "in",
  width = 6, height = 6
)
...
mergercases_base <- data.frame(
  acquirer = c(rep("Republic", 4), rep("Santek", 3), rep("WasteConn", 2), "WMADS"),
  target = c("Santek", "WasteConn", "WMADS", "Regional", "WasteConn", "WMADS", "Regional", "WMADS", "Regional", "Regional")
)

mergercases <- expand_grid(
  acquirer = c("Republic", "Santek", "WasteConn", "WMADS"),
...
mkt_mergersweep_firm <- bind_rows(lapply(mkt_mergersweep, function(x) {
  return(x$firm)
}))
mkt_mergersweep <- bind_rows(lapply(mkt_mergersweep, function(x) {
  return(x$sum)
}))
...
trashfakemergerplot <- ggplot(data = filter(mkt_mergersweep, Acquirer != "WMADS" & # Target !="Regional" &#Acquirer=="Republic" & Target=="Santek" &
  ...
  geom_point() +
  geom_line() +
  facet_grid(Target ~ Acquirer, scales = "free") +
  theme_bw() +
  ylab("% of Pre-merger Expenditures") +
  xlab("Merger") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank(), legend.position = "bottom") +
  geom_hline(yintercept = 0, linetype = "dashed")

ggsave(file.path(basedir, "output", "trashfakemerger.png"), trashfakemergerplot, height = 7, width = 7)
...
trashfakemergerplotalt <- ggplot(data = filter(mkt_mergersweep, Acquirer != "WMADS" & # Target !="Regional" &#Acquirer=="Republic" & Target=="Santek" &
  ...
  mutate(
    Merger = paste(Acquirer, Target, sep = ":"),
    Merger = reorder(Merger, ifelse(name == "Consumer Harm" & Unintegrated == "Base", -value, NA), max, na.rm = TRUE),
    Unintegrated = relevel(Unintegrated, "Base")
  ), aes(x = Merger, y = value, color = name, group = name)) +
  geom_point() +
  geom_line() +
...
ggsave(file.path(basedir, "output", "trashfakemergeralt.png"), trashfakemergerplotalt, height = 7, width = 7)
...
trashinterestingmergerbar <- ggplot(data = filter(mkt_mergersweep, !(Acquirer == "Santek" & Target == "Republic" & Unintegrated == "Base") &
  ...
  filter(Merger %in% c(
    "Republic/Santek", # "Santek/Republic",
    "Santek/WasteConn", "Santek/WMADS", "WMADS/Regional"
  )), aes(
  y = Merger, x = value # ,fill=Type
)) +
  # facet_grid(~Unintegrated,scales = "free") +
  ...
  xlab("% of Pre-merger Expenditures") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "goldenrod") +
...
ggsave(filename = file.path(basedir, "output", "trashinterestingmergerbar.png"), trashinterestingmergerbar, width = 6.5, height = 4.5)


trashinterestingsantrep <- ggplot(data = filter(
  mkt_mergersweep, Acquirer %in% c("Santek", "Republic") & Target %in% c("Santek", "Republic") &
    name %in% c("Consumer Harm", "Disposal Benefit", "Collection Benefit") &
  ...
  mutate(
    Merger = ifelse(Acquirer %in% c("Republic") & Target %in% c("Santek"), paste(Acquirer, Target, sep = "/"),
  ...
  filter(Merger %in% c("Republic/Santek", "Santek/Republic", "Santek/WasteConn", "Santek/WMADS", "WMADS/Regional")), aes(
  y = Unintegrated, x = value # ,fill=Type
)) +
  facet_grid(~Type, scales = "fixed") +
  geom_bar(
    stat = "identity", fill = "lightgrey",
  ...
  theme(legend.position = "bottom", axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_discrete(limits = rev) +
  ylab("Merger Arrangement")

ggsave(filename = file.path(basedir, "output", "trashinterestingsantrep.png"), trashinterestingsantrep, width = 7, height = 4)
...
trashinterestingfirmbar <- ggplot(
  data = filter(mkt_mergersweep_firm, Unintegrated == "Base") %>%
  ...
  facet_grid(Merger ~ Level + Effect, scales = "free_x") +
  geom_bar(
    stat = "identity",
    position = position_dodge2(reverse = TRUE)
    # position="stack"
...
ggsave(filename = file.path(basedir, "output", "trashinterestingfirmbar.png"), trashinterestingfirmbar, width = 7, height = 7)
write_csv(mkt_mergersweep, file.path(basedir, "output", "mkt_mergersweep.csv"), na = "")
write_csv(mkt_mergersweep_firm, file.path(basedir, "output", "mkt_mergersweep_firm.csv"), na = "")
...
bargsweep <- function(thissim, barg = 0.5, who = c("Santek:Republic", "WasteConn:Republic", "All", "Base")) {
  ...
  minBarg_bert <- optimize(function(x) {
    minD(c(x, barg), relevant = relevantPre)
  }, lower = -1e4, upper = -1e-4)
...
mkt_bargsweep <- lapply(
  c("Santek:Republic", "WasteConn:Republic", "All", "Base"),
  function(x) {
    res_bargsweep <- lapply(seq(0.1, 0.9, 0.1), function(z) {
      bargsweep(simres_vert, barg = z, who = x)
    })
...
trashbargplot <- ggplot(data = na.omit(filter(mkt_bargsweep, scenario == "All" & name %in% c(
  "Consumer Harm" # ,"Disposal Benefit","Collection Benefit"
))), aes(
  x = barg, y = value,
  # color=scenario,
  group = scenario
  ...
  theme_bw() +
  ylab("% of Pre-merger Expenditures") +
  xlab("Bargaining Power") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank(), legend.position = "bottom") +
  geom_hline(
    data = mkt_bargsweep %>%
      filter(scenario == "Base" & name %in% c(
        "Consumer Harm" # ,"Disposal Benefit","Collection Benefit"
      )),
    aes(yintercept = value), linetype = "dashed"
  ) + # labs(title="The Effect of Changing Bargaining Power on Simulation Outcomes",
...
ggsave(file.path(basedir, "output", "trashbarg.png"), trashbargplot, height = 7, width = 7)
...
trashelastplot <- ggplot(data = na.omit(filter(mkt_elastsweep, name %in% c(
  "Consumer Harm"
  # ,"Disposal Benefit","Collection Benefit"
))), aes(
  x = shareElast, y = value, # color=name,
  group = name
)) +
  geom_point() +
  geom_line() +
  theme_bw() +
  ylab("% of Pre-merger Expenditures") +
  xlab("% Outside Share (Market Elasticity)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank(), legend.position = "bottom") +
...
ggsave(file.path(basedir, "output", "trashelast.png"), trashelastplot, width = 7, height = 7)
...
trashelastplot_alt <- ggplot(data = na.omit(filter(mkt_elastsweep, name %in% c("Consumer Harm", "Disposal Benefit", "Collection Benefit"))), aes(x = factor(abs(round(as.numeric(mktElast), 2))), y = value, color = name, group = name)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank(), legend.position = "bottom") +
  labs(
    title = "The Effect of Changing Outside Share on Simulation Outcomes",
    subtitle = "Outside share of 0 depicts base case."
  ) +
  scale_color_brewer(type = "seq", palette = "YlGnBu", direction = -1)
...
nestcases <- expand_grid(nestParm = seq(0.1, 1, 0.1), nest = c(
  "integrated",
  "landfill"
)) # %>%
...
mkt_nestsweep_firm <- bind_rows(lapply(mkt_nestsweep, function(x) {
  return(x$firm %>% mutate(product = rownames(x$firm)))
}))
mkt_nestsweep <- bind_rows(lapply(mkt_nestsweep, function(x) {
  return(x$sum)
}))
...
trashnestplot <- ggplot(data = filter(
  mkt_nestsweep,
  # nest=="All" &
  name %in% c(
    "Consumer Harm"
...
ggsave(file.path(basedir, "output", "trashnest.png"), trashnestplot, height = 7, width = 7)
mkt_nestsweep_firm$nestParm <- 1 - mkt_nestsweep_firm$nestParm
write_csv(mkt_nestsweep_firm, file.path(basedir, "output", "mkt_nestsweep_firm.csv"), na = "")
