...
psummary_hhi.bw <- ggplot(
  ...
  scale_color_manual(values = seq_palette)+
  facet_grid(vert~Merger,scale="free_y") +
  theme_bw() + xlab("Post-merger HHI") + ylab("Consumer Harm (%)") + geom_hline(yintercept=0,color="goldenrod",linetype="dashed") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),legend.position="bottom")+
...
psummary_heat <- ggplot(filter(res_mkt,#abs(cv/mktrev.pre*100)<=80 &
              hhidelta<=2500 &
              nestParm == "0"  ) %>%
...
pvertincumb.bw <-  ggplot(res_mkt , aes(y=cvtrun,
                                           x=vert#color=Cost
                                           #,color=Outcome
                                           )
                                           ) +
...
psummary.bw <-  ggplot(res_mkt, aes(y=cvtrun,
...
pprofits_barg <- pprofits_barg + geom_segment(
  ...
  geom_text(aes(x=x,y=y),color="black",label="equal power",angle=90,
            data=data.frame(x=4.5,y=40,#Outcome=factor( "Consumer" ,levels=unique(res_mkt.logit$Outcome)),
  ...
  geom_text(aes(x=x,y=y),color="black",label="more retailer power",
            data=data.frame(x=6.75,y=20,#Outcome=factor( "Consumer" ,levels=unique(res_mkt.logit$Outcome)),
...
png("output/surplussum_hhi.png",width = 13, height = 13, units = "in", res=300)
print(psummary_hhi.bw)
dev.off()


png("output/surplussum_deltahhi.png",width = 13, height = 13, units = "in", res=300)
