...
basedir_calib <- file.path(Sys.getenv("HOME"), "Projects", "bargaining_calibrate")
source(file.path(basedir_calib, "code", "MonteCarlo.R"))
...
market <- function(nfirms.down = 3, # # of downstream firms
  ...
  if (!is.null(hhidelta) && largeMerger) {
    stop("non-null hhidelta is not compatible with largeMerger")
  }

  if (any(is.na(nestParm) | nestParm >= 1 | nestParm < 0)) {
    stop("nestParm must be >=0 and <1")
  }
  if (length(nestParm) > 2) {
    stop("nestParm must be length less than or equal to 2")
  }
  ...
  ids <- expand.grid(
    up.firm = as.character(1:nfirms.up), up.products = as.character(1:nprods.up),
  ...
  if (missing(nests) && max(nestParm, na.rm = TRUE) > 0) {
    if (isNestedTogether == "same") {
      if (ownerPost %in% c("up", "both")) {
        nests <- ids$up.firm == 1 | ids$up.firm == 2
      } else if (ownerPost == "down") {
        nests <- ids$down.firm == 1 | ids$down.firm == 2
      } else if (ownerPost == "vertical") nests <- ids$up.firm == 1 | ids$down.firm == 1
    } else if (isNestedTogether == "different") {
      if (ownerPost %in% c("up", "both")) {
        nests <- ids$up.firm == 1 | ids$up.firm == 3
      } else if (ownerPost == "down") {
        nests <- ids$down.firm == 1 | ids$down.firm == 3
      } else if (ownerPost == "vertical") nests <- ids$up.firm == 1 | ids$down.firm == 2
    }
  } else {
    nests <- rep(TRUE, nrow(ids))
  }

  nests <- droplevels(factor(nests, levels = c(TRUE, FALSE)))


  if (length(nests) != nrow(ids) | any(is.na(nests))) {
    stop(" 'nests' length must be equal total number of products and cannot contain NAs")
  }

  if (nlevels(nests) > 1) {
    nestMat <- tcrossprod(model.matrix(~ -1 + nests))
  } else {
    nestMat <- matrix(1, nrow = nrow(ids), ncol = nrow(ids))
  }
  ...
  if (ownerPost %in% c("vertical", "both") || nfirms.vert > 0) {
    if (nfirms.vert > 0) {
      vertFirms <- 2:(nfirms.vert + 1)
    }
    if (ownerPost == "both") {
      vertFirms <- vertFirms + 1
      vertFirms <- c(1:2, vertFirms)
    }
    ...
    if (ownerPost %in% c("vertical")) {
      bargparmPost[1] <- 1
    } # all bargaining power resides with the retailer in a vertical  merger
    if (ownerPost %in% c("up", "both")) {
      bargparmPost[ids$up.firm == 1 & ids$down.firm == 2] <- 1
    } # post-merger, u1 gives d2 a discount
    if (ownerPost %in% c("down", "both")) {
      bargparmPost[ids$up.firm == 2 & ids$down.firm == 1] <- 1
    } # post-merger u2 gives d1 a discount
  }
  ...
  if (nlevels(ids$up.firm) > 1) {
    ...
    ownerPre.up <- matrix(1, nrow = nrow(ids), ncol = nrow(ids))
  }
  ...
  if (nlevels(ids$down.firm) > 1) {
    ...
    ownerPre.down <- matrix(1, nrow = nrow(ids), ncol = nrow(ids))
  }
  ...
  mkt <- list(
    up = list(
      nfirms = nfirms.up, nproducts = nprods.up, ownerPre = ownerPre.up, ownerPost = ownerPost.up,
      bargparm = bargparm, mcshare = mcshare.up, isParty = isParty.up
    ),
    down = list(
      nfirms = nfirms.down, nproducts = nprods.down, shares = sharesDown,
...
simMarket.1st <- function(mkt, cost_type = c("constant", "linprod", "quadprod", "linquad", "quadlin", "lincons", "conslin", "linfirm", "consparty", "linparty"), ...) {
  ...
  if (cost_type == "constant") {
    mcParm <- list(up = upMC, down = downMC)
  } else if (cost_type == "linprod") {{ mcParm <- list(up = sharesDown * M / upMC, down = sharesDown * M / downMC) }} else if (cost_type == "quadprod") {{ mcParm <- list(up = (sharesDown * M)^2 / upMC, down = (sharesDown * M)^2 / downMC) }} else if (cost_type == "linquad") {{ mcParm <- list(up = (sharesDown * M) / upMC, down = (sharesDown * M)^2 / downMC) }} else if (cost_type == "quadlin") {{ mcParm <- list(up = (sharesDown * M)^2 / upMC, down = (sharesDown * M) / downMC) }} else if (cost_type == "linfirm") {{ mcParm <- list(up = as.vector(ownerPre.up %*% sharesDown) * M / upMC, down = as.vector(ownerPre.down %*% sharesDown) * M / downMC) }} else if (cost_type == "lincons") {{ mcParm <- list(up = (sharesDown * M) / upMC, down = downMC) }} else if (cost_type == "conslin") {{ mcParm <- list(up = upMC, down = (sharesDown * M) / downMC) }} else if (cost_type == "consparty") {{ mcParm <- list(
    up = ifelse(isParty.up, upMC, (sharesDown * M) / upMC),
    down = ifelse(isParty.down, downMC, (sharesDown * M) / downMC)
  ) }} else if (cost_type == "linparty") {{ mcParm <- list(
    up = ifelse(isParty.up, (sharesDown * M) / upMC, upMC),
    down = ifelse(isParty.down, (sharesDown * M) / downMC, downMC)
  ) }}

  ## create a container to store upstream and downstream data
  mkt <- list(
    up = c(mkt$up, list(price = upPrices, margin = 1 - upMC / upPrices, mc = upMC, mcParm = mcParm$up)),
    down = c(mkt$down, list(
      alpha = alpha, price = downPrices,
...
summary.1st <- function(mkt, market = FALSE,
                        # vertical = ifelse(length(mkt$vertical)>0 & mkt$vertical$vertFirms==1, TRUE,FALSE),
                        useEst = FALSE,
                        level = c("all", "up", "down")) {
  if (useEst) {
